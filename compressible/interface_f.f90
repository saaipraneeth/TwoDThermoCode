...
!f2py intent(out) :: F
  ...
  do j = jlo-1, jhi+1
     do i = ilo-1, ihi+1
        ...
        ! step III: Compute the HLLC flux
        ...
        if (S_r <= 0.0d0) then
           ...
           call consFlux(idir, pres, idens, ixmom, iymom, iener, nvar, &
                         U_state, F(i,j,:))
...
!            ! Here Pressure - based wave speed estimates; Toro - 
...
subroutine consFlux(idir, pres, idens, ixmom, iymom, iener, nvar, U_state, F)        
  ...
  p = pres(U_state(iener), U_state(idens))
