...
class peng_robinson_fluid():
    ...
    '''
    def __init__(self, setup,grid):
      self.setup    = setup
      self.Rcst     = 8.3144621	                #J/(mol*K)
      self.Nxc      = grid.Nxc_withgc		# Loading number of grid points (with ghost cells pre-accounted for) from grid object
      ...
      self.viscous   = setup.settings['viscous']
      self.Prandtl  = setup.settings['Prandtl']
      self.species  = setup.settings['species']      
      self.N_species= setup.settings['N_species']
      self.visc_Tref= setup.settings['viscosity_Sutherland_Tref']
      self.visc_muref= setup.settings['viscosity_Sutherland_muref']
      self.visc_Sref= setup.settings['viscosity_Sutherland_Sref']

      print self.visc_Tref
